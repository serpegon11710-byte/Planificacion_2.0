...
Permite al usuario crear nuevos proyectos o modificar proyectos existentes mediante gestión manual. Al crear un proyecto, el sistema automáticamente crea un item por defecto que a su vez genera una planificación "Sin planificar".
...
flowchart TD
   ...
   F --> G[Crear planificacion Sin planificar]
...
- **Creación automática de Planificación:** El item automático genera una planificación "Sin planificar"
...
8. **Sistema crea automáticamente una Planificación "Sin planificar" para el item**
...
3. Sistema recorre **todas** las planificaciones de **todos** los items del proyecto y detecta bloqueos por RE-3 (Completada) y/o RE-4 (ocurrencias materializadas en planificaciones **periódicas**)
4. Si hay bloqueos: sistema muestra aviso con la **lista completa** de planificaciones impeditivas (`IdentificablePorUsuario` + motivo por cada una). Ver FA-4 y RN-2.6. Fin sin borrar.
5. Si no hay bloqueos: sistema muestra confirmación: "¿Eliminar proyecto '{nombre}'? Esto eliminará también todos sus items y planificaciones"
6. Usuario confirma
7. Sistema elimina el proyecto y todos sus elementos relacionados (cascada)
8. Sistema muestra confirmación
...
### FA-4: Planificaciones que impiden eliminar el proyecto (pasos 3–4 de Eliminar)

**Casos posibles por planificación** (pueden combinarse en la misma entrada):

| Motivo | Condición | Acción que debe tomar el usuario |
|--------|-----------|----------------------------------|
| Completada | RE-3 | Editar planificación → Pendiente (UC-01.4) |
| Ocurrencias gestionadas (solo periódicas) | RE-4 | Anular o restaurar en gestión por planificación (UC-02.4) |
| Ambos | RE-3 y RE-4 | Ambas acciones, en cualquier orden, hasta vaciar bloqueos |

**Flujo:**
1. Sistema agrega a la lista **cada** planificación bloqueante (no solo la primera).
2. Sistema muestra aviso **inequívoco** (RN-2.6): por cada entrada, el **`IdentificablePorUsuario`** completo y el **motivo(s)**.
3. Ejemplo orientativo:

   > No se puede eliminar el proyecto «Marketing 2026». Prepare estas planificaciones antes de reintentar:
   > • Proyecto «{proyecto}» · Item «{item}» · {tipo_planificacion} · «{observaciones}» · {fecha_inicio}–{fecha_fin} · {hora} — Completada
   > • Proyecto «{proyecto}» · Item «{item}» · {tipo_planificacion} · «{observaciones}» · … — Completada; 2 ocurrencias gestionadas

4. Caso de uso finaliza sin eliminar nada. Código de error: `ELIMINACION_PROYECTO_BLOQUEADA` (payload RE-5 con `identificable_por_usuario`).
...
### RN-2.4: RE-3 y RE-4 bloquean la eliminación del proyecto
No se puede eliminar un proyecto si alguna de sus planificaciones está Completada (RE-3) o, si es **periódica**, tiene ocurrencias materializadas (RE-4). Objetivo: evitar borrados masivos accidentales. El usuario debe revertir manualmente con UC-01.4 y UC-02.4 antes de reintentar.

### RN-2.6: Aviso inequívoco de planificaciones bloqueantes (RE-5)
Si el borrado del proyecto está bloqueado, el mensaje al usuario **debe** listar todas las planificaciones impeditivas mostrando el **`IdentificablePorUsuario`** de cada una (ver [planificaciones.md](../entidades/planificaciones.md)) y el **motivo** (Completada, ocurrencias gestionadas con cantidad, o ambos). No se admite un aviso genérico sin detallar cuáles son.

### RN-2.5: Eliminación en Cascada
Cuando se cumple RN-2.4, al eliminar un Proyecto se eliminan automáticamente:
...
| **Planificación** | Opcional configurar | Se crea "Sin planificar" por defecto |
...
- Usa valores por defecto (nombre del proyecto, "Sin planificar", etc.)
...
- **Entidad:** [proyectos.md](../entidades/proyectos.md), [items.md](../entidades/items.md)

## Trazabilidad C4

| Zona critica N4 | Rol |
|-----------------|-----|
| [ZC-4](../diagramas-c4/c4-nivel-4/pseudocodigo/zc-4-orquestacion-aplicacion.md) | Creacion proyecto + item + Sin planificar |
